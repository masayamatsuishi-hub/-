...
import { calcAchievementRate, calcProteinTargetGrams, sumDailyTotals } from '../utils/nutrition'
...
export default function HistoryPage() {
  ...
  useEffect(() => {
    if (user) fetchHistoryRange(user.id, fromDate, today)
          ...
          return (
            <li key={date} className="rounded-xl border border-slate-800 bg-slate-900/50 px-4 py-3">
              <div className="mb-1 flex items-center justify-between">
                ...
                <span className="text-xs text-slate-500">{logs.length}件の記録</span>
              </div>
              {logs.length === 0 ? (
